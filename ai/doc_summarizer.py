...
load_dotenv()
# configure Bedrock client without using profile_name
...
def summarizer(prompt_data) -> str:
    ...
    :param prompt_data: This is the prompt along with the respective chunk of text, at the end it contains all summary chunks combine>
...
def Chunk_and_Summarize(uploaded_file) -> str:
    ...
    summary = ""
    # looping through each chunk of text we created
    for index, chunk in enumerate(texts):
        # gathering the text content of that specific chunk
        chunk_content = chunk.page_content
        # creating the prompt that will be passed into Bedrock
        prompt = f"""\n\nHuman: Provide a detailed summary for the chunk of text provided to you:
        Text: {chunk_content}
        \n\nAssistant:"""
        # passing the prompt into the summarizer function
        summary += summarizer(prompt)
        # printing out the number of tokens contained in each chunk
        print(f"\n\nNumber of tokens for Chunk {index + 1} with the prompt: {num_tokens_from_string(prompt)} tokens")
        print("-------------------------------------------------------------------------------------------------------")
        # after we have generated the summaries of each chunk of text, we pass it into the final summary prompt
    final_summary_prompt = f"""\n\nHuman: You will be given a set of summaries from a document. Create a cohesive 
    summary from the provided individual summaries. The summary should be very detailed and at least 10 pages. 
    Summaries: {summary}
            \n\nAssistant:"""
